"""Honing-5c: U/W occupancy opt — write wgmma acc straight to gmem (output viewed as [NT,BT,H,DV]),
dropping the 32KB f32 sZ staging + its write-loops/barriers. Higher occupancy -> faster. vs FLA recompute."""
import os, sys
import cutlass
...
@cute.kernel
def _grid_uw3(gAi, gK, gV, gBeta, gG, gU, gW):
    ...
    layB = smu.make_smem_layout(cutlass.BFloat16, LayoutEnum.ROW_MAJOR, (DV, BT))
    ...
    for it in cutlass.range_constexpr(DV * BT // 128):
        idx = it * 128 + tid
        sBt[idx // BT, idx % BT] = (sBeta[idx % BT] * sV[idx % BT, idx // BT].to(cutlass.Float32)).to(cutlass.BFloat16)
    cute.arch.barrier()
    accU, tAi, tB = smu.partition_fragment_ABC(thr, (BT, DV, BT), sAi, sBt)
    smu.gemm(mma, accU, tAi, tB, zero_init=True)
    cute.autovec_copy(accU, thr.partition_C(gU[chunk, None, head, None]))   # direct to gmem
    cute.arch.barrier()

    for it in cutlass.range_constexpr(DK * BT // 128):
        idx = it * 128 + tid
        sBt[idx // BT, idx % BT] = (sBeta[idx % BT] * cute.math.exp(sG[idx % BT]) * sK[idx % BT, idx // BT].to(cutlass.Float32)).to(cutlass.BFloat16)
    cute.arch.barrier()
...
def _run(mAi, mK, mV, mB, mG, mU, mW):
    ...
    _grid_uw3(mAi, mK, mV, mB, mG, mU, mW).launch(grid=(nb, 1, 1), block=(128, 1, 1))


_c = {}

def uw3(Ai, k, v, beta, g, T):
    NT = T // BT
    U = torch.empty(T, H, DV, device=DEV, dtype=torch.float32)
    W = torch.empty(T, H, DK, device=DEV, dtype=torch.float32)
    U4 = U.view(NT, BT, H, DV)
    W4 = W.view(NT, BT, H, DK)
    args = [from_dlpack(Ai, assumed_align=16), from_dlpack(k, assumed_align=16), from_dlpack(v, assumed_align=16),
            ...
            from_dlpack(U4, assumed_align=16), from_dlpack(W4, assumed_align=16)]
    if T not in _c:
...
def main():
    print("Device:", torch.cuda.get_device_name(0), "| U/W occupancy-opt (direct gmem write) vs FLA recompute")
    ...
    for T in S.SEQLENS:
        ...
        U, W = uw3(Ai_blocks, kc, vc, bc, gc, T)
        Ai0 = Ai_blocks[0].float()
        Uref0 = Ai0 @ (bc[:BT, 0, None].float() * vc[:BT, 0, :].float())
        c0 = cos(U[:BT, 0, :], Uref0)
        t_mine = time_fn(lambda: uw3(Ai_blocks, kc, vc, bc, gc, T))
        t_fla = time_fn(lambda: recompute_w_u_fwd(k, v, beta, g, Ai_fla.to(torch.bfloat16), None))
        print("%7d | %9.4f %11.4f %7.2fx   (U[0] cos=%.4f)" % (T, t_mine, t_fla, t_fla / t_mine, c0))
