"""Honing-9: uw6 + bf16 output (match FLA, halve write bandwidth) + pre-scaled Ai — feed V/K DIRECTLY, no transpose-build.
...
def _grid_uw6(gAi, gK, gV, gBeta, gG, gU, gW):
    ...
    accU_bf = cute.make_rmem_tensor(accU.shape, cutlass.BFloat16)
    accU_bf.store(accU.load().to(cutlass.BFloat16))
    cute.autovec_copy(accU_bf, thr.partition_C(gU[chunk, None, head, None]))
    cute.arch.barrier()
    ...
    accW_bf = cute.make_rmem_tensor(accW.shape, cutlass.BFloat16)
    accW_bf.store(accW.load().to(cutlass.BFloat16))
    cute.autovec_copy(accW_bf, thr.partition_C(gW[chunk, None, head, None]))
...
def main():
    print("Device:", torch.cuda.get_device_name(0), "| U/W MN-major + bf16-out vs FLA")
    ...
    for T in S.SEQLENS:
        ...
        U, W = uw7(Ai_blocks, kc, vc, bc, gc, T)
        ...
        t_mine = time_fn(lambda: uw7(Ai_blocks, kc, vc, bc, gc, T))
